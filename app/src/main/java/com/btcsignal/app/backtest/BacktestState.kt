...
import com.btcsignal.app.engine.PriceZoneConfig
    ...
    fun start(context: Context) {
        ...
        job = scope.launch {
            try {
                ...
                // Read the current block list and the price-zone rule at run-start, so a
                // strategy blocked (vestigial now, but kept for compatibility) or a zone
                // edited on the Settings screen is honored here too, not just on the Live
                // path.
                val settings = AppContainer.settingsRepository(appContext).settingsFlow.first()
                val summary = engine.run(
                    db,
                    periodDays,
                    blockedStrategyIds = settings.blockedStrategyIds,
                    priceZoneConfig = PriceZoneConfig(
                        redLowPct = settings.priceZoneRedLowPct,
                        redHighPct = settings.priceZoneRedHighPct,
                        greenLowPct = settings.priceZoneGreenLowPct,
                        greenHighPct = settings.priceZoneGreenHighPct,
                        winUsd = settings.priceZoneWinUsd,
                        lossUsd = settings.priceZoneLossUsd,
                        waitEnabled = settings.priceZoneWaitEnabled
                    )
                ) { p -> progress.value = p }
