...
## 24. Opposite-direction rule, hard 4:00 cutoff, and 5s Waiting (spec: "BTC 5-Minute
Signal Logic — Exact Modification Specification")

A full audit of the price-zone rule (the entire signal engine — see §21/§23 above for
its own history) against this spec found `PriceZoneScanner.kt` had drifted from it in
three ways, none of which were actually implemented despite being requested previously:

1. **No opposite-direction gating at all (spec sections 2, 4, 10).** The scanner fired
   RED whenever price moved into the Red Zone and GREEN whenever it moved into the
   Green Zone, completely independent of what the candle itself did during minutes
   0-2. This is the most serious gap: a candle that opened RED could still fire another
   RED signal, which the spec explicitly forbids ("the actual signal must always be
   opposite to the initial direction... there must be no same-direction signal").
2. **The `4:00` cutoff was inclusive, not exclusive.** `onTick` rejected a tick only when
   `tMillis > windowEndMillis`, which let a signal fire at exactly `4:00` — an existing
   unit test asserted this as correct behavior. Spec section 3 is explicit that the
   window is `2:00 <= elapsed_time < 4:00` and section 9 lists `4:00` under "Signal: NO".
3. **No "5s Waiting" setting anywhere** — not in `AppSettings`, not in the Settings
   screen, not in the scanner, for either Live or Backtest (spec section 8).

**Fixes applied:**

- **`PriceZoneScanner.kt`**: added `InitialDirection` (`GREEN`/`RED`/`NONE`) and the
  shared `determineInitialDirection(candleOpen, priceAtMinute2Close)` helper (single
  source of truth, spec section 19); added `setInitialDirection(...)`, called once per
  candle the instant minute 2 closes. `onTick` now resolves a single `targetDirection`
  — the opposite of whatever `setInitialDirection` recorded — and only ever tests
  *that* zone; a `NONE` initial direction (minute-2 close exactly equal to candle open)
  permanently disables signal generation for that candle, per section 10. The window
  check is now `tMillis < windowStart || tMillis >= windowEnd`, making `4:00` exclusive.
  `PriceZoneConfig` gained `waitEnabled: Boolean`, and `onTick` gained the section-8.2
  confirm/cancel state machine: the first qualifying tick starts a pending timer instead
  of firing; the first tick at or after that entry's `+5000ms` mark decides purely from
  its own price (still inside the zone -> fire; outside -> cancel, keep monitoring for a
  fresh entry). Because every tick is independently subject to the same `< windowEnd`
  check, a confirmation due at/after `4:00` simply never receives a tick to evaluate it
  on, so waiting can never push a signal past the cutoff (tested explicitly, see below).
- **`SettingsRepository.kt`**: added persisted `priceZoneWaitEnabled: Boolean` (default
  `false`) to `AppSettings`, threaded through `setPriceZoneConfig` as one more field in
  the same atomic write as the rest of the price-zone rule.
- **`SettingsScreen.kt`**: added a "5s Waiting" toggle to the Signal Zones card, seeded
  from persisted state and included in the same Save call as the zone edges and
  win/loss amounts (not written through immediately, matching every other field in this
  card).
- **`LiveMonitoringService.kt`**: `onKlineUpdate` now calls
  `priceZoneScanner.setInitialDirection(...)` synchronously, directly off the
  Checkpoint-B event, in the same call stack as the tick that reads `onTick` above —
  not inside the async `processAggregatorEvents` launch, which would otherwise race
  against ticks for the same candle arriving before the initial direction was actually
  recorded. `priceZoneConfig` (the `@Volatile` copy rebuilt from Settings for each new
  candle) now also carries `waitEnabled`.
- **`BacktestEngine.kt`**: added a `minuteOffset == 1` branch (the close of the bucket's
  2nd 1-minute sub-candle, i.e. exactly `2:00`) that calls the same
  `determineInitialDirection` helper and feeds it to that candle's scanner, before the
  existing `minuteOffset == 2 || 3` scan branch ever runs for it — Live and Backtest
  now share not just the zone/window logic but the initial-direction logic too.
  `BacktestState.kt` threads `priceZoneWaitEnabled` through the `PriceZoneConfig` it
  builds from Settings.
- **`PriceZoneScannerTest.kt`**: rewritten — every existing case now sets an initial
  direction before asserting on `onTick` (previously none did, since the rule didn't
  exist); the old test asserting a fire at exactly `4:00` was replaced with one
  asserting no fire at `4:00` but a fire at `3:59`; new cases cover same-direction
  suppression, `NONE`/never-set initial direction, `determineInitialDirection` itself,
  and the full 5s-waiting state machine (confirm, cancel, re-entry after a cancelled
  attempt, and an entry too close to `4:00` to ever confirm).

**Files touched**: `PriceZoneScanner.kt`, `SettingsRepository.kt`, `SettingsScreen.kt`,
`LiveMonitoringService.kt`, `BacktestEngine.kt`, `BacktestState.kt`,
`PriceZoneScannerTest.kt`.

**Not verified**: as with every prior revision (§18), no Android SDK/Gradle/network
access is available in this environment, so `./gradlew test` / `assembleDebug` have not
actually been run here — brace/paren balance and a full manual re-read of every touched
file were used instead. Please run the unit tests in Android Studio before trusting
this in Live.

## 25. Gold-Zone rebrand: name, icon, version, and side-by-side install with the previous build

User request (chat handoff): change the app's display name, version, and icon (recolored
to red-gold), while remaining installable **alongside** the previous "Reversal-Zone"
build already on the device rather than overwriting it. No engine, scanner, or logic
files were touched — this is a branding/identity-only change, same category as the
`applicationId` bump already documented in `app/build.gradle.kts`'s own inline comments
for the Alpha → Reversal-Zone transition.

- **`applicationId`**: `com.btcsignal.app.reversal` → `com.btcsignal.app.goldzone`.
  Android keys an installed app's identity off `applicationId`, not display name or
  `namespace`, so this is what actually makes the two APKs coexist as separate installed
  apps rather than one overwriting the other.
- **`versionCode`** / **`versionName`**: `5` / `"4.0.0-pricezone"` →
  `6` / `"5.0.0-goldzone"`, continuing the existing per-revision suffix convention.
- **`app_name`** (`strings.xml`, the launcher label): `"Hadlas Reversal Zone"` →
  `"Hadlas Gold Zone"`.
- **Launcher icon** (`ic_launcher_background.xml` / `ic_launcher_foreground.xml`):
  recolored to red-gold — deep red-black background (`#1A0604`, distinct from the
  original teal-black, Alpha's violet, and Reversal-Zone's teal-black), gold up-arrow
  (`#E8B93D`) + red down-arrow (`#E6394F`) in place of Reversal-Zone's orange/cyan, and
  a pale-gold badge (`#F5D67B`) in place of the plain white one. Same silhouette/shape as
  every prior build, only the palette changed, so it reads as the same app family at a
  glance while still being visually distinguishable in the app drawer from any
  already-installed earlier build.

**Not changed**: the in-app UI theme (`ui/theme/Color.kt`, the "gothic cyberpunk"
palette used by every screen) and the `LiveMonitoringService` foreground-notification
title (`"BTCUSDT Signal monitoring"`) were left as-is — the request was specifically
about the launcher name/icon/version, not an in-app re-theme. Flagging this in case a
full in-app red-gold re-theme was actually wanted too.

**Files touched**: `app/build.gradle.kts`, `strings.xml`, `ic_launcher_background.xml`,
